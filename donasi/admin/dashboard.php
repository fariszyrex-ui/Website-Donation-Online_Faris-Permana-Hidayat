...
$total_campaign = $pdo->query("SELECT COUNT(*) AS total FROM campaign")
    ->fetch(PDO::FETCH_ASSOC)["total"];

$campaign_aktif = $pdo->query("SELECT COUNT(*) AS total FROM campaign WHERE status = 'aktif'")
    ->fetch(PDO::FETCH_ASSOC)["total"];

$total_donatur = $pdo->query("SELECT COUNT(*) AS total FROM donatur")
    ->fetch(PDO::FETCH_ASSOC)["total"];

$total_donasi = $pdo->query("SELECT COUNT(*) AS total FROM donasi")
    ->fetch(PDO::FETCH_ASSOC)["total"];

$total_dana = $pdo->query("
...
$campaign_terbaru = $query->fetchAll(PDO::FETCH_ASSOC);
...
<div class="wrap">
    ...
    <div class="page-head">
        <div>
            <h1>Dashboard</h1>
            <p>Selamat datang kembali, <?= e($_SESSION["nama"] ?? "Admin") ?>.</p>
        </div>

        <div class="page-actions">
            <a class="btn btn-ghost" href="berita.php#tambah-berita">
                Tulis berita
            </a>
            <a class="btn btn-primary" href="kelola_campaign.php#tambah-campaign">
                Campaign baru
            </a>
        </div>
    </div>
    ...
    <section class="stats" aria-label="Ringkasan">
        ...
        <div class="stat">
            <div>
                ...
                <div class="stat-value" data-count="<?= (int) $total_campaign ?>"><?= number_format($total_campaign, 0, ",", ".") ?></div>
                <div class="stat-note"><?= (int) $campaign_aktif ?> sedang aktif</div>
            </div>
        </div>
        ...
        <div class="stat">
            <div>
                ...
                <div class="stat-value" data-count="<?= (int) $total_donatur ?>"><?= number_format($total_donatur, 0, ",", ".") ?></div>
        ...
        <div class="stat">
            <div>
                ...
                <div class="stat-value" data-count="<?= (int) $total_donasi ?>"><?= number_format($total_donasi, 0, ",", ".") ?></div>
        ...
        <div class="stat">
            <div>
                ...
                <div class="stat-value<?= strlen(rupiah($total_dana)) > 15 ? ' long' : '' ?>" data-count="<?= (int) $total_dana ?>" data-prefix="Rp "><?= rupiah($total_dana) ?></div>
                <div class="stat-note">Donasi berhasil</div>
            </div>
        </div>
    ...
    <section class="panel">
        ...
        <div class="panel-head">
            ...
            <a href="kelola_campaign.php">Kelola semua campaign</a>
        </div>

        <?php if (count($campaign_terbaru) > 0): ?>
            ...
            <div class="table-wrap">
                <table class="data">
                    ...
                    <tbody>
                        <?php foreach ($campaign_terbaru as $c): ?>

                            <?php
                            $persen = persen($c["dana_terkumpul"], $c["target_donasi"]);
                            $aktif  = strtolower($c["status"]) === "aktif";
                            ?>

                            <tr>
                                <td>
                                    <div class="cell-title"><?= e($c["judul"]) ?></div>
                                </td>
                                ...
                                <td class="progress-cell">
                                    ...
                                    <div class="progress-meta">
                                        <span>
                                            <strong><?= rupiah($c["dana_terkumpul"]) ?></strong>
                                            <span class="muted">dari <?= rupiah($c["target_donasi"]) ?></span>
                                        </span>
