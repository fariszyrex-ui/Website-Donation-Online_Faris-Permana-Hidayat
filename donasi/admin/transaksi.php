...
if (!function_exists("kelas_status")) {
    function kelas_status($status)
    {
        $s = strtolower(trim($status));

        if (in_array($s, ["berhasil", "sukses", "success", "lunas", "paid", "selesai"])) {
            return "badge-aktif";
        }

        if (in_array($s, ["pending", "menunggu", "diproses", "proses"])) {
            return "badge-pending";
        }

        if (in_array($s, ["gagal", "ditolak", "dibatalkan", "batal", "failed", "expired", "kadaluarsa"])) {
            return "badge-gagal";
        }
...
$query = $pdo->prepare("
    ...
    FROM donasi
    INNER JOIN donatur
        ON donasi.id_donatur = donatur.id_donatur
    INNER JOIN campaign
...
/* Ringkasan & daftar status (dihitung dari data di atas) */
...
<div class="wrap">
    ...
    <div class="page-head">
        <div>
            ...
            <p>Riwayat seluruh donasi yang masuk.</p>
        </div>
    </div>


    <section class="stats cols-3" aria-label="Ringkasan">

        <div class="stat">
            <div class="stat-label">Total transaksi</div>
            <div class="stat-value" data-count="<?= (int) $total_transaksi ?>"><?= number_format($total_transaksi, 0, ",", ".") ?></div>
            <div class="stat-note">Semua status</div>
        </div>

        <div class="stat">
            <div class="stat-label">Transaksi berhasil</div>
            <div class="stat-value" data-count="<?= (int) $jumlah_berhasil ?>"><?= number_format($jumlah_berhasil, 0, ",", ".") ?></div>
            <div class="stat-note"><?= (int) ($total_transaksi - $jumlah_berhasil) ?> belum berhasil</div>
        </div>

        <div class="stat">
            <div class="stat-label">Dana masuk</div>
            <div class="stat-value<?= strlen(rupiah($dana_berhasil)) > 15 ? ' long' : '' ?>" data-count="<?= (int) $dana_berhasil ?>" data-prefix="Rp "><?= rupiah($dana_berhasil) ?></div>
            <div class="stat-note">Dari donasi berhasil</div>
        </div>
    ...
    <section class="panel">
        ...
        <?php if ($total_transaksi > 0): ?>
            ...
            <div class="toolbar" data-table="tabel-transaksi">
                ...
                <div class="segmented" data-filter-status>
                    ...
                    <?php foreach ($daftar_status as $st): ?>
                        <button type="button" data-value="<?= e($st) ?>"><?= e(ucfirst($st)) ?></button>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="table-wrap">
                ...
                <table class="data" id="tabel-transaksi">
                    ...
                    <tbody>
                        ...
                        <?php foreach ($transaksi as $data): ?>
                            ...
                            <tr data-status="<?= e(strtolower(trim($data["status"]))) ?>">
                                ...
                                <td>
                                    <div class="cell-title"><?= e($data["nama_donatur"]) ?></div>
                                </td>

                                <td>
                                    <?= e($data["judul_campaign"]) ?>
                                </td>

                                <td class="num">
                                    <strong><?= rupiah($data["nominal"]) ?></strong>
                                </td>

                                <td>
                                    <?= e($data["metode_pembayaran"]) ?>
                                </td>

                                <td>
                                    <span class="badge <?= kelas_status($data["status"]) ?>">
                                        <?= e(ucfirst($data["status"])) ?>
                                    </span>
                                </td>

                                <td>
                                    <div><?= tgl_indo($data["tanggal_donasi"]) ?></div>

                                    <?php if (jam_indo($data["tanggal_donasi"]) !== ""): ?>
                                        <div class="small muted"><?= e(jam_indo($data["tanggal_donasi"])) ?></div>
                                    <?php endif; ?>
                                </td>
            ...
            <div class="no-result" hidden>Tidak ada transaksi yang cocok.</div>

        <?php else: ?>

            <div class="empty">
                <h3>Belum ada transaksi</h3>
                <p>Donasi yang masuk dari donatur akan tercatat di sini.</p>
            </div>
